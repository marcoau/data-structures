var BloomFilter = function(m, hashObjs){
  //m: number of

  var _.storage = [];
  //populating _.storage with m zeros.
  for(var i = 0; i < m; i++){
    _.storage.push(0);
  }
  //after this, _.storage is complete with m zeros.

  this.add = function(value){
    for(var i in hashObjs){
      _.storage[(hashObjs[i](value))] = 1;
    }
  };

  this.query = function(value){
    for(var i in hashObjs){
      if(_.storage[hashObjs[i](value)]){return true;}
    }
    return false;
  };
};
...
var hashes = {
  hash1: function(i){
    var hash = CryptoJS.SHA1(i, { outputLength: 512 }).toString();
    return parseInt('0x' + hash, 16) % 18;
  },
  hash2: function(i){
